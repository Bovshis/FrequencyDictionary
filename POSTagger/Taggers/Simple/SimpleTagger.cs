...
namespace POSTagger.Taggers.Simple
{
    public class SimpleTagger : ITagger
    {
        ...
        public string Tag(Corpus corpus, string token)
        {

            if (!corpus.WordMap.ContainsKey(token) || !corpus.WordMap.ContainsKey(token.ToLower()))
            {
                return token.Length == 1 ? "^" : "NN";
            }

            var value = corpus.WordMap[token];
            var sb = new StringBuilder();
            foreach (string tag in corpus.TagMap[value].Where(tag => !tag.Contains(',')))
            {
